...
from .api_operate import api_get_info_func, api_get_last_post, api_search_func, apt_get_explore_post
from .config import Config
from .sql_operate import query_UUID_by_name, insert_or_update_data, query_mateId_by_name, query_name_by_name
from .tools import time_to_format
...
async def get_info_func(args: Message = CommandArg()):
    ...
    try:
        message = MessageSegment.text(f'''角色名：{info['角色名']}\nmateID：{info["角色ID"]}\n年龄：{info["年龄"]}\n个人简介：{info["个人简介"]}\n''')
...
async def get_last_post_func(args: Message = CommandArg()):
    ...
    for i in data:
        ...
        formatted_time = time_to_format(dtCreate, any_config.UTC)
...
async def remind_update_func(args: Message = CommandArg()):
    ...
    result = "已经" + days + str(hours) + '小时' + str(minutes) + '分钟' + str(seconds) + "秒" + "没更新啦！！！"
...
async def get_help_func(args: Message = CommandArg()):
    message = MessageSegment.text("🌟AnyMate小助手 " + "v" + any_config._plugin_version + "\n")
    message += MessageSegment.text("🔹/anyhelp 展示此帮助\n")
    message += MessageSegment.text("🔸/信息 <角色名> 展示指定角色相关信息\n")
    message += MessageSegment.text("🔹/动态 <角色名> [数量] 展示指定角色指定数量的最新动态\n")
    message += MessageSegment.text("🔸/搜索 <角色名> [数量] 搜索角色并展示指定数量的结果\n")
